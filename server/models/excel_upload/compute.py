from shared.models.interfaces import ExcelUploadInput as Input, Output
...
class Compute:
    def __init__(self, input: Input) -> None:
        ...
        self.bucket_name = "sukoon-user-data"
    ...
    def compute(self) -> Output:
        formatted_data = self.format_data(self.input.data)
        file_buffer = self.save_to_excel(formatted_data)
        url = self.upload_to_s3(file_buffer, self.input.file_name)
        self.delete_old_files('engagement_data_', keep_latest=1)

        return Output(
            output_details={"file_url": url},
            output_status=OutputStatus.SUCCESS,
            output_message="Excel file uploaded successfully"
        )
